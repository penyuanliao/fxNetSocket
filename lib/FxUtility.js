...
FxUtility.prototype.findOutSocketConnected = function (client, chunk, self) {
    ...
    let forwarded = {
        // "X-Real-IP": client.socket.remoteAddress.replace("::ffff:", ""),
        // "X-Forwarded-For": request_headers["x-forwarded-for"],
        // "X-From-Cdn": request_headers["x-from-cdn"],
        // "Ali-CDN-Real-IP": request_headers["ali-cdn-real-ip"],
        // "BB-FORWARDED": request_headers["bb-forwared"]
    };
    ...
    else if (typeof request_headers["x-from-cdn"] != "undefined" && request_headers["x-from-cdn"] != "") {
        forwarded["X-From-Cdn"] = request_headers["x-from-cdn"];
        forwarded["Ali-CDN-Real-IP"] = request_headers["ali-cdn-real-ip"];
    }
    else if (typeof request_headers["bb-forwared"] != "undefined" && request_headers["bb-forwared"] != "") {
        forwarded["BB-FORWARDED"] = request_headers["bb-forwared"];
    } else {
        forwarded = null;
    }
