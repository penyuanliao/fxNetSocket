...
logger.prototype.configure = function (options) {
    ...
    if (typeof options.fileNoDate != "undefined") this.fileNoDate = options.fileNoDate;
...
logger.prototype.log = function (d) {
    ...
    var path = (typeof this.historyLog == "undefined") ? historyLog : this.historyLog;
    ...
    if (this.logFile && this.folderExists) {
        ...
        if (firstOne) {
            log_file[this.fileName] = {
                num:0,
                file: fs.createWriteStream(path + '/'+ formatDate(this.fileNoDate) + this.fileName + '.log',{ flags:'a' })
            };
            // init default size
            fs.stat(path + '/'+ formatDate(this.fileNoDate) + this.fileName + '.log', function (err, data) {
                if (!err) {
        ...
        if( fullSize && this.doShiftFile != true) {
            if (this.fileSort == "asc") {
                ...
                if (num >= this.maximumFileCount) {
                    ...
                    log_file[this.fileName] = {
                        num:0,
                        file: fs.createWriteStream(path + '/'+ formatDate(this.fileNoDate) + this.fileName + '_' + num + '.log',{ flags:'w' })
                    }
                } else {
                    log_file[this.fileName] = {
                        num:num,
...
logger.prototype.shiftFile = function (path) {
    ...
    var oldPath = path + '/'+ formatDate(this.fileNoDate) + this.fileName + '.log';
    var newPath = path + '/'+ formatDate(this.fileNoDate) + this.fileName + '_' + (num) + '.log';
    ...
    var onRename = function onRename(err, data) {
        ...
        log_file[self.fileName].file = fs.createWriteStream(path + '/'+ formatDate(self.fileNoDate) + self.fileName + '.log',{ flags:'w' });
        setTimeout(function () {
            oldFile.end();
        }, 100);

        if (num >= self.maximumFileCount) {
            var delPath = path + '/'+ formatDate(self.fileNoDate) + self.fileName + '_' + (num - self.maximumFileCount) + '.log';
...
logger.prototype.shiftFileDesc = function (path) {
    ...
    var onDescrFile = function onDescrFile() {
        ...
        if (j == 0) {
            oldPath = path + '/'+ formatDate(self.fileNoDate) + self.fileName + '.log';
        } else {
            oldPath = path + '/'+ formatDate(self.fileNoDate) + self.fileName + '_' + j + '.log';
        }
    ...
    var onRename = function onRename(err, data) {
        if (err) {
            ...
            fs.unlink(path + '/'+ formatDate(self.fileNoDate) + self.fileName + '_' + (j+1) + '.log', function (err, data) {
                onDescrFile();
            })
            return;
        }
        ...
        if (j < 0) {
            ...
            log_file[self.fileName].file = fs.createWriteStream(path + '/'+ formatDate(self.fileNoDate) + self.fileName + '.log',{ flags:'w' });
            if (num >= self.maximumFileCount) {
                var delPath = path + '/'+ formatDate(self.fileNoDate) + self.fileName + '_' + (self.maximumFileCount+1) + '.log';
                fs.unlink(delPath, onUnlink);
            } else {
                self.doShiftFile = false;
            }
