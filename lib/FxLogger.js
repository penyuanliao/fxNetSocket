...
function logger(options) {
    ...
    this.maximumFileCount = (typeof options != "undefined" && typeof options.fileMaxCount == "number") ? options.fileMaxCount : 100;
    ...
    if (this.logFile) {
        ...
        if (firstOne) {
            log_file[this.fileName] = {
                num:0,
                file: fs.createWriteStream(path + '/'+ formatDate() + this.fileName + '_' + 0 + '.log',{ flags:'a' })
            };
        }
