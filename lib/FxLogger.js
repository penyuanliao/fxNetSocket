...
logger.prototype.configure = function (options) {
    ...
    if (typeof options.level == 'string') {
        ...
        logger.debugLevel = indx;
        if (indx == -1) logger.debugLevel = 0;
    }
...
logger.prototype.log = function (d) {
    ...
    if (level == -1)
        ...
        str = times + util.format.apply(util, arguments.slice(1, arguments.length)) + '\r\n';// win:\r\n linux:\n mac:\r
    
    if (this.logFile) {

        const firstOne = (typeof log_file === 'undefined');
        const fullSize = log_file[this.fileName]._writableState.length > this.maximumFileSize;
