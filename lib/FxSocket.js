...
const brCompress      = ifdef('iltorb').compress;
const brCompressSync  = ifdef('iltorb').compressSync;
/***
...
FxSocket.prototype.handeshake = function (chunk) {
    ...
    if (typeof readHeaders["sec-websocket-protocol"] != "undefined") {
        ...
        for (var i = 0; i < subProtols.length; i++) {
            if (compressCodec[subProtols[i]]) {
                readHeaders["sec-websocket-protocol"] = subProtols[i];
...
FxSocket.prototype.read = function (data) {
    ...
    if (this.mode === 'ws') {
        ...
        if (opcode === 1) {
            ...
            obj.msg = new Buffer(this.protocol.msg).toString();
