...
function FxSocket(socket, bufferPool)
{
    ...
    this.mode = '';
    this.payload = (typeof bufferPool !== 'undefined') ? bufferPool : Buffer.allocUnsafe(1024 * 32);
    this.compressCodec = {
        "br":false,
        "aes":true,
    }
...
FxSocket.prototype.write = function (data, enc) {
    ...
    if (this.pindingDrain === true && this.backpressure === true) {
        this.drainQueue.push([data, enc]);
        return false;
    }
    /**/
    var flushed = true; // flushed success on kernel buffer
    var len = 0;
...
FxSocket.prototype.read = function (data) {
    ...
    if (this.mode === 'ws') {
        ...
        if (opcode === 1 || opcode === 0) {
            ...
            setTimeout(function () {
                this.socket.write(this.emit_websocket('', 9))
            }.bind(this), 10000)
        }
        // opcode 0x01 Text
...
FxSocket.prototype.close = function () {
    ...
    if (this.mode === 'ws' && this.connecting) {
        try {
            const closeEvt = this.emit_websocket(JSON.stringify({"NetStatusEvent":"NetConnection.Connect.Closed"}),1);
            if (this.baseEvtShow) this.socket.write(closeEvt);
...
FxSocket.prototype.read_websocket = function(data) {
    var proto = this.encoder.readFraming(data);
    return proto;
}
...
FxSocket.prototype.emit_websocket = function(data, opcode, fin) {
    ...
    if (bfsize > payload.length && isBuf == false) {
    ...
    }else
    {
        ...
        var _buffer = this.encoder.writeFraming(fin,opcode,false,dPayLoad);
        return Buffer.concat([_buffer, dPayLoad], _buffer.length + bfsize);
    }

    //var payload = Buffer.from(data);
    var buffer = this.encoder.writeFraming(fin,opcode,false,payload);
...
FxSocket.prototype.ws_compress_async = function (data, opcode, callback) {
    ...
    brCompress(src.payload, function (error, output) {
        var header = this.encoder.writeFraming(true,src.opcode,false, output);
