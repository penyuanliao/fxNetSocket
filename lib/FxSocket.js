...
var FxSocket = function(socket, bufferPool)
{
    ...
    if (this._heartbeatEnabled){
        this.startupHeartbeat()
    }
};

FxSocket.prototype.startupHeartbeat = function () {
    var self = this;
    this.cookies = 0;
    var todo = function () {
        console.log('socket heartbeat cookies:', self.cookies);
        self.cookies++;
        if (self.cookies > 10) { // 5 mins
            self.close();
            todo = null;
            self.clearHeartbeat();
        }else {
            self.heartbeat = setTimeout(todo,30000);
        }
    }
