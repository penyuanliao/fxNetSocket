...
function emit_websocket(data,opcode) {
    ...
    }else
    {
        ...
        var _buffer = parser.protocols.writeFraming(true,opcode,false,dPayLoad);
        return Buffer.concat([_buffer, dPayLoad], _buffer.length + bfsize);
    }

    //var payload = new Buffer(data);
    var buffer = parser.protocols.writeFraming(true,opcode,false,payload);
...
FxSocket.prototype.__defineGetter__("connecting", function () {
   if (this.socket._connecting && this.writable = true) {
       return true;
   };
    return false;
});
