...
Headers.prototype.readHeaders = function (chunk) {
    ...
    var reqMethod = lines[0].toString().match(/^(GET|POST)? (.+)[\/]? HTTP\/\d\.\d$/i); // WS protocol namespace endpoint no '/'
    // [?=\/] 結尾不包含
    reqMethod = (reqMethod == null) ? lines[0].toString().match(/^(GET|POST)? (.+) HTTP\/\d\.\d$/i) + "/" : reqMethod;
...
Headers.prototype.writeHandshake = function (reqHeaders) {
    ...
    var resHeaders = [
        ...
        'Now: ' + ("rc." + mapping.hostname() + (process.env.NODE_CDID || "1.15.7"))
    ];
...
Protocols.prototype.writeFraming = function (fin, opcode, masked, payload) {
    ...
    if (typeof _meta === 'undefined' || _meta.length < len) {
        ...
        _meta = Buffer.alloc(2 + (len < 126 ? 0 : (len < 65536 ? 2 : 8)) + (masked ? 4 : 0));
    }
    meta = _meta;
...
module.exports = exports = {'headers':new Headers(), Encoder: Protocols,'protocols':new Protocols()}
