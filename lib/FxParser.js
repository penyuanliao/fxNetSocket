...
Headers.prototype.readHeaders = function (chunk) {
    ...
    var reqMethod = lines[0].toString().match(/^(GET|POST|PUT|DELETE)? (.+)[\/]? HTTP\/\d\.\d$/i); // WS protocol namespace endpoint no '/'
    // [?=\/] 結尾不包含
    reqMethod = (reqMethod == null) ? lines[0].toString().match(/^(GET|POST|PUT|DELETE)? (.+) HTTP\/\d\.\d$/i) + "/" : reqMethod;
