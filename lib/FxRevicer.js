...
function FxRevicer(client, mode) {
    ...
    this.forcedBinary = client.forcedBinary;
    ...
    /** @property {FxReaderPool} reader 讀取streaming資料 */
...
FxRevicer.prototype.onHandle = function (proto) {
    ...
    if (proto.compressed) {
        ...
        stream.flush(zlib.Z_SYNC_FLUSH, function () {
            if (proto.opcode == 8) {
                console.log('onHandle() opcode:', proto.opcode);
                return;
            }
...
FxRevicer.prototype.emitMessage = function (data) {
    if (this.glListener) {
        this.client.delegate.emit("message", {'client':client,'data':data});
    } else {
...
FxRevicer.prototype.parser = function (cb) {
    ...
    let payload_length = part % 128; //  if 0-125, that is the payload length
 ...
 * @return {boolean}
 */
FxRevicer.prototype.recordPing = function (data) {
    ...
    if (data.indexOf('"ping":') != -1) {
        ...
        const ping = this.client.probeResult(json);
        if (this.glListener)
        {
            this.emit("ping", {client:this.client, ping:ping});
        }
        else
        {
            this.client.emit("ping", ping);
        }
        return true;
    }
    return false;
