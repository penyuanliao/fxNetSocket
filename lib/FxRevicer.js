...
FxRevicer.prototype.createDecompress = function () {
    ...
    if (typeof this.stream == "undefined") {
        ...
        this.stream.on("data", function (data) {
            if (self.isRelease) return;
            endpoint.emit("message", (endpoint.forcedBinary ? data : data.toString()));
        });
    }
    return this.stream;
};
FxRevicer.prototype.onHandle = function (proto) {
    ...
    if (proto.compressed) {
        ...
        stream.flush(zlib.Z_SYNC_FLUSH, function () {
            ...
            console.log('onHandle.flush', proto.opcode);
            if (proto.opcode == 8) stream.close();
        });
    } else {
        if (this.isRelease) return;
        if (this.forcedBinary) {
            this.client.emit("message", proto.msg);
        } else {
            this.client.emit("message", proto.msg.toString());
        }
    }
};
