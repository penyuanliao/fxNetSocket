...
exports.getConfiguration = function (name, consolelog) {
    var obj = new iConfiguration(name);
    ...
    if (obj.files.constructor === Object) {
        if (typeof arguments[1] != "undefined"){
            arguments[1]("info","Loading Configuation file:%s choose name:'%s', ip:'%s'", name, obj.files["pipelines"].name, obj.files["pipelines"].ip);
        }else {
...
function iConfiguration(name) {
    ...
    this.files = this.loadFiles('../configuration/' + fileName + env + type);
