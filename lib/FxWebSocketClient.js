    ...
    function addUpdateData(mode, client) {
        ...
        client.socket.on('data', function (chunk) {
            ...
            if (mode === fxStatus.websocket) {
                count = 0;
                while (chunk.length > 0 && count < splitsReceiveLimitCount) {
                    count++;
                ...
                if (count === splitsReceiveLimitCount) {
                    client.emit("error", util.format("Splits the received ByteBufs on limit max count %s.", splitsReceiveLimitCount));
                    client.close();
