        ...
        client.socket.on('data', function (chunk) {
            ...
            if (mode === fxStatus.websocket) {
                while (chunk.length > 0) {
                    ...
                    if (self.pingEnable == true && self.recordPing(client, data) == true) continue;
            ...
            self.emit("data", chunk);
            if (self.pingEnable == true && self.recordPing(client, data) == true) return;
            if (typeof data != "undefined") self.emit('message',data);
        });
