...
function FxConnection(port, option) {
    ...
    this.app.on('connection', function(socket) {
        ...
        socket.once('data', function (data) {
            ...
            client.emit("connect");
            if (self.nPingPong && mode == fxStatus.websocket) socket.write(client.emit_websocket('', 9));
        });
        socket.on('close', function () {
            chunkBuffer = undefined;
            if (revicer) socket.unpipe(revicer.reader);
        });
        /**
        ...
        const revicer = new FxRevicer("websocket", client); // parser read
        // const sender = new FxSender(client); // wire format
